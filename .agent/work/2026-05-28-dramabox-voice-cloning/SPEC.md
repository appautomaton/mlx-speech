...
4. **Reference conditioning** (`AudioConditionByReferenceLatent` equivalent) — patchify the reference latent, compute its positions (target positions + 0.5 s offset), **append** ref tokens to the end of the latent sequence, set their `denoise_mask=0` (kept clean — note `denoise_mask` drives BOTH the loop re-blend AND the per-token AdaLN timestep; see step 5), and build an **asymmetric** self-attention bias `[B,1,N+M,N+M]` (target→ref allowed = 0.0; ref→target blocked = log-space `-inf`).
5. **DiT conditioning plumbing** — thread TWO per-token signals for the appended ref tokens: **(a)** the asymmetric `attention_mask` from `LTXModel.__call__` → `LTXBlock.__call__` → `audio_attn1` (the `mask` arg is already accepted by `LTXAttention` and forwarded to SDPA); and **(b) per-token timesteps** `timesteps = denoise_mask * sigma` into the AdaLN path, so ref tokens (`denoise_mask=0`) are modulated as *clean* at timestep 0 instead of at the noisy sigma. Upstream both signals ride the per-step `Modality` (`timesteps_from_mask`, `_prepare_timestep` → `[B,T,coeff*hidden]`); a scalar sigma corrupts ref-token identity (artifacts/echoing). Concat positions use the existing `precompute_split_freqs_from_positions` in one call. The no-ref path (`denoise_mask` all-ones) must stay bit-stable after this change.
...
5. **[runtime]** DiT runs with appended ref tokens + mask + **per-token timesteps** (ref tokens modulated at timestep 0, not the noisy sigma) with no shape error; the no-ref path (`denoise_mask` all-ones) is **bit-stable** vs the scalar-sigma baseline; `clear_conditioning` removes exactly M tokens; decoded output length equals the no-ref run.
