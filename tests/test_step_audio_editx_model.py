...
class TestCache:
    def test_cache_grows_across_decode_steps(self):
        cfg = Step1Config(
            hidden_size=8,
        ...
        out1 = model(input_ids=x)
        mx.eval(out1.logits)
        assert out1.logits.shape == (1, 3, 32)
        assert len(out1.cache) == 1
        assert out1.cache[0].key.shape == (1, 3, 1, 4)

        next_ids = mx.array([[4]], dtype=mx.int32)
        out2 = model(input_ids=next_ids, cache=out1.cache)
        mx.eval(out2.logits)
        assert out2.cache[0].key.shape == (1, 4, 1, 4)
        assert out2.cache[0].value.shape == (1, 4, 1, 4)
